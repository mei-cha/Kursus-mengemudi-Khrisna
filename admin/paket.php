...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_paket'])) {
        ...
        if ($stmt->execute([$nama_paket, $deskripsi, $durasi_jam, $harga, $tipe_mobil, $termasuk_teori, $termasuk_praktik, $maksimal_siswa, $tersedia])) {
            $success = "Paket kursus berhasil ditambahkan!";
        } else {
            $error = "Gagal menambahkan paket kursus!";
        }
    }
    elseif (isset($_POST['edit_paket'])) {
        ...
        if ($stmt->execute([$nama_paket, $deskripsi, $durasi_jam, $harga, $tipe_mobil, $termasuk_teori, $termasuk_praktik, $maksimal_siswa, $tersedia, $id])) {
            $success = "Paket kursus berhasil diupdate!";
        } else {
            $error = "Gagal mengupdate paket kursus!";
        }
    }
}
...
if (isset($_GET['delete'])) {
    ...
    $check_stmt = $db->prepare("SELECT COUNT(*) as count FROM pendaftaran_siswa WHERE paket_kursus_id = ?");
    $check_stmt->execute([$id]);
    $usage = $check_stmt->fetch(PDO::FETCH_ASSOC);
    
    if ($usage['count'] > 0) {
        $error = "Tidak bisa menghapus paket ini karena sudah digunakan dalam pendaftaran!";
    } else {
        $stmt = $db->prepare("DELETE FROM paket_kursus WHERE id = ?");
        if ($stmt->execute([$id])) {
            $success = "Paket kursus berhasil dihapus!";
        } else {
            $error = "Gagal menghapus paket kursus!";
        }
    }
}
...
if (isset($_GET['toggle'])) {
    ...
    if ($stmt->execute([$id])) {
        $success = "Status ketersediaan berhasil diubah!";
    } else {
        $error = "Gagal mengubah status ketersediaan!";
    }
}
...
<body class="bg-gray-100">
            ...
            <main class="flex-1 overflow-y-auto p-6">
                ...
                <div class="bg-white rounded-lg shadow mb-6">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <h3 class="text-lg font-medium text-gray-900" id="formTitle">Tambah Paket Kursus Baru</h3>
                    </div>
                    <div class="p-6">
                        <form method="POST" id="packageForm">
                            <input type="hidden" name="id" id="editId">
                            <input type="hidden" name="edit_paket" id="editMode" value="0">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="number" name="durasi_jam" id="durasi_jam" required min="1"
                                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                                   placeholder="10">
                                        </div>
                ...
                <div class="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-6">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3">
                            <h4 class="text-lg font-medium text-blue-800">Tips Penetapan Harga</h4>
                            <ul class="mt-2 text-sm text-blue-700 list-disc list-inside space-y-1">
                                <li>Paket Pemula: Rp 1.000.000 - Rp 2.000.000 (6-10 jam)</li>
                                <li>Paket Lengkap: Rp 2.000.000 - Rp 3.000.000 (12-15 jam)</li>
                                <li>Paket Executive: Rp 3.000.000+ (16+ jam dengan fitur tambahan)</li>
                                <li>Harga manual biasanya 10-20% lebih mahal dari matic</li>
                                <li>Include persiapan ujian SIM untuk paket premium</li>
                            </ul>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function editPackage(id) {
            fetch(`get_paket_data.php?id=${id}`)
                ...
                .then(data => {
                    ...
                    document.getElementById('durasi_jam').value = data.durasi_jam;
                    ...
                    document.getElementById('submitButton').innerHTML = '<i class="fas fa-save mr-2"></i>Update Paket';
        ...
        function resetForm() {
            ...
            document.getElementById('submitButton').innerHTML = '<i class="fas fa-plus mr-2"></i>Tambah Paket';
        ...
        document.getElementById('durasi_jam').addEventListener('input', function() {
            const durasi = parseInt(this.value) || 0;
            const hargaInput = document.getElementById('harga');
            
            if (durasi > 0) {
                // Basic pricing calculation (100k per hour as base)
                const basePrice = durasi * 100000;
        ...
        document.getElementById('packageForm').addEventListener('submit', function(e) {
            ...
            const durasi = parseInt(document.getElementById('durasi_jam').value);
            
            if (harga < 100000) {
                alert('Harga terlalu rendah! Minimum Rp 100.000');
                e.preventDefault();
                return;
            }
            
            if (durasi < 1) {
                alert('Durasi harus minimal 1 jam!');
                e.preventDefault();
                return;
            }
